...
class TaskManager {
    ...
    remindAboutUnstartedTask(task) {
        ...
        if (!task.hasStarted() && now - taskCreationTime > oneWeek) {
            ...
            console.log(`Reminder sent to ${this.student.name} for unstarted task: ${task.title}.`);
        }
    }
    /**
    ...
    checkDeadlineApproaching(task) {
        ...
        if (deadlineTime - now <= twoDays) {
            ...
            console.log(`Reminder sent to ${this.student.name} for Deadline approaching for task: ${task.title}.`);
        }
    }
}

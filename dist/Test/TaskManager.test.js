...
describe('TaskManager', () => {
  ...
  it('should throw error if teacher is not set when creating task', () => {
    ...
    expect(() => taskManager.createTask(task)).toThrow(
      'Student or teacher must be set before creating a task.',
    )
  ...
  it('should throw error if task with same ID already exists', () => {
    ...
    taskManager.assignTaskToStudent(task, teacher, student)
    const newTaskWithSameId = new Task_1.Task(
      '1',
      'Math',
      'Test',
      'Teacher',
    ...
    expect(() => taskManager.createTask(newTaskWithSameId)).toThrow(
      'A task with the ID 1 already exists.',
    )
